...
df_rf = pd.read_excel('CleanedData_TSFA_Daily.xlsx', 'T-Bill', index_col=0)
...
def sr_annu(x_old,rf_old):
    ...
    annu_ret_expected = (ret_expected+1)**260-1
    annu_std_dev = std_dev * np.sqrt(260)
...
def getSumStat(data, rf = df_rf['T-Bill'], rounding = 2):
    ...
    ss_temp['Annu. Ave Return(%)'] = np.round(((data.mean()+1)**252-1)*100, rounding)
    ss_temp['Annu. Std(%)'] = np.round(data.std()*np.sqrt(252)*100, rounding)
...
def graphicalAnalysis(dataset, start_date = '2000-01-01', end_date = '2020-05-31', 
    ...
    for i in range(n_labels + 1):
        temp = names[labels == i]
        print('Cluster %i: %s' % ((i + 1), ', '.join(temp)))
        cluster_list.append(temp.tolist())
        #print(cluster_list)
    ...
    # Title of the plot
    title = 'Graphical Network Analysis of Selected Firms over the Period '+start+' to '+end+' (Daily)'
    ...
    if (display_IndRet):
        print('Individual Stock Performance over the Period '+ start+' to '+end+' (Daily):')
